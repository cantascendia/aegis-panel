...
> **TBD-1**:默认 4 角色是否合理?是否需要补 `auditor`(只读全局,合规审计专用)/
> `reseller`(分销代理,仅管理自己拉来的子用户)?S-RB session 第一周确认,
> 通过即写入 D-XXX。
...
> **TBD-2**:permission_key naming 风格是否锁定 `<scope>:<verb>:<target>`?
> 备选:`<resource>.<action>` (`users.read`)、纯下划线 (`users_read_list`)。
> 当前 :-segmented 与 AWS IAM / GCP IAM 一致,可读性最好,但要 S-RB 第一周
> 走 1 个 admin endpoint 的 PoC 验证 grep 噪声 / 字符串误拼率。
...
> **TBD-3**:Phase 2 → Phase 3 过渡期(双保险并行)长度?候选:2 周 / 4 周 /
> 1 个 minor release。S-RB session 视实际 endpoint 数量与漏覆盖风险拍板。
