/**
 * NilouIcon — inline-SVG icon system, currentColor-themed.
 *
 * Ported verbatim from design Atoms.jsx. Stroke 1.5px, round caps,
 * 24x24 viewBox. New icons match this stroke weight.
 *
 * Named NilouIcon (not Icon) to avoid collision with lucide-react's
 * default Icon component. Use this for marketing-aligned glyphs;
 * lucide-react remains the default for utility icons in operator
 * forms / tables.
 */
import type { FC, ReactElement } from 'react';

export type NilouIconName =
    | 'home' | 'nodes' | 'chart' | 'tag' | 'receipt' | 'ticket' | 'user'
    | 'book' | 'bell' | 'search' | 'plus' | 'arrow' | 'check' | 'copy'
    | 'download' | 'refresh' | 'logout' | 'settings' | 'shield' | 'globe'
    | 'chevronDown' | 'chevronRight' | 'qr' | 'key' | 'heart' | 'rss'
    | 'code' | 'mail' | 'pin' | 'flame';

export interface NilouIconProps {
    name: NilouIconName;
    size?: number;
}

const PATHS: Record<NilouIconName, ReactElement> = {
...
export const NilouIcon: FC<NilouIconProps> = ({ name, size = 18 }) => (
    <svg
        width={size}
        height={size}
        viewBox="0 0 24 24"
